...
SELECT        -- Final table
  pubs.id,
  pubs.title.preferred AS pub_title,
  pubs.journal.title AS journal_title,
  publisher.name AS publisher_name,
  pubs.category_hra.values AS category,
  altmetrics.score AS alt_score,
  CASE WHEN ARRAY_LENGTH(funder_countries) = 0 THEN NULL ELSE ARRAY_LENGTH(funder_countries) END AS num_countries_involved,
